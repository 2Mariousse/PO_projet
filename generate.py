...
def generate(file_name: str, draw_pile: list[int], n_ascendants: int, n_descendants: int):
    ...
    with open(f"{file_name}.pddl", "w") as f:
        ...
        f.write(f"        {" ".join(f"c{value}" for value in draw_pile)} - card\n")
        f.write(f"        {" ".join(f"c_start_{i}" for i in range(n_ascendants + n_descendants))} - card\n")
        f.write(f"    )\n")
        ...
        for i in range(n_ascendants + n_descendants):
            f.write(f"        (top_pile_card pile{i} c_start_{i})\n")
        ...
        for i in range(n_ascendants):
            f.write(f"        (= (value c_start_{i}) 1)\n")
        for i in range(n_ascendants, n_ascendants + n_descendants):
            f.write(f"        (= (value c_start_{i}) 100)\n")
